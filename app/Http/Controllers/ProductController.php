...
class ProductController extends Controller
{
    ...
    public function saveInventory(Request $request){
        ...
        if($request->inventory_id != ''){
            ...
            $insert =   DB::select("INSERT INTO total_inventory_items (inventory_id, product_id, price,unit,quantity) SELECT '$request->inventory_id',product_id,price,unit,qty FROM temp_product_out WHERE type = 8 and temp_product_out.user_id = 'Auth::user()->id' ");
            ...
            $insert =   DB::select("INSERT INTO total_inventory_items (inventory_id, product_id, price,unit,quantity) SELECT '$total_id',temp_product_out.product_id,temp_product_out.price,temp_product_out.unit,temp_product_out.qty FROM temp_product_out WHERE type = 8 ");
            $delete = TempProductout::where('type',8)->where('user_id',Auth::user()->id)->delete();
        }
